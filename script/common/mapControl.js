...
window.mapControl = {
  ...
  initMap: function () {
    ...
    this.subscribeParentWinMessage(); // 订阅message
  ...
  subscribeParentWinMessage: function () {
    window.addEventListener("message", function (event) {
      if (event && event.data) {
        try {
          ...
          if (dataInJson.type === "realtime") {
            TrackAction.switchmanagetab(0);
          } else {
            TrackAction.switchmanagetab(1);
            TrackAction.changedatetime(dataInJson.date);
            TrackAction.selecttrack(dataInJson.eleName);
          }
    ...
    this.TrafficControl.prototype.initialize = function (map) {
      ...
      traffic.onclick = function (e) {
        if (traffic.className.indexOf("trafficOn") > -1) {
          ...
          traffic.className = traffic.className.replace(
            /trafficOn/,
            "trafficOff"
          );
        } else {
