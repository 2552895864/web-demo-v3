...
var TrackStore = Reflux.createStore({
      ...
      function (data) {
        if (data.status === 0) {
          data.columns.map(function (item) {
            that.data.column.push(
              item.column_desc !== "" ? item.column_desc : item.column_key
            );
  ...
  onBoundsearchentity(display) {
    ...
    if (
      !this.data.switchBounds ||
      display === false ||
      this.data.managetab === 1
    ) {
    ...
    for (let i = 0; i < width; i++) {
      for (let j = 0; j < height; j++) {
        let leftButtom = pointToLngLat(
          new BMap.Pixel(
            pixelSouthWest.x + widthUnit * i,
            pixelSouthWest.y + heightUnit * j
          )
        );
        let rightTop = pointToLngLat(
          new BMap.Pixel(
            pixelSouthWest.x + widthUnit * (i + 1),
            pixelSouthWest.y + heightUnit * (j + 1)
          )
        );
        boundsArr.push(
          leftButtom.lat +
        ...
        Urls.jsonp(Urls.boundsearchEntity, params, function (data) {
          if (data.status === 0) {
            data.entities.map(function (eitem, eindex) {
              ...
              let entity_status = 0;
              if (
                Commonfun.getOnlineStatus(eitem.latest_location.loc_time) === 0
              ) {
              ...
              inBoundsEntity.push({
                ...
                point: [
                  eitem.latest_location.longitude,
                  eitem.latest_location.latitude,
                ],
          ...
          if (
            ++inBoundsEntityCount === width * height &&
            inTimestamp === that.data.boundsearchTimestamp
          ) {
  ...
  onSelectcar: function (entity_name, entity_status, entity_id) {
    var that = this;
    if (
      that.data.selectCar.entity_name === undefined &&
    ...
    entity_status =
      entity_status !== undefined
        ? entity_status
        : that.data.selectCar.entity_status;
              ...
              Urls.jsonp(Urls.getAddress, paramsGeo, function (dataGeo) {
                ...
                that.data.column_key.map(function (keyitem, index) {
                  if (keyitem === "_provider") {
                    ...
                    temp[index] = [
                      that.data.column[index] + ":",
                      item[keyitem] !== undefined ? item[keyitem] : "无",
                    ];
                  }
                });
                ...
                var lnglat =
                  item.latest_location.longitude.toFixed(6) +
                  "," +
                  item.latest_location.latitude.toFixed(6);
                var address = "地址未解析成功";
                if (dataGeo.result.formatted_address !== "") {
                  address = dataGeo.result.formatted_address;
                } else {
                  address =
                    dataGeo.result.addressComponent.city +
                    ", " +
                    location_desc.result.addressComponent.country;
                }
                that.data.selectCompleteEntities[index] = {
                  ...
                  status: Commonfun.getInfoWindowStatus(
                    item.latest_location.speed,
                    item.latest_location.loc_time,
                    item.latest_location.direction
                  ),
                  infor: [
                    [
                      "状态:",
                      Commonfun.getInfoWindowStatus(
                        item.latest_location.speed,
                        item.latest_location.loc_time,
                        item.latest_location.direction
                      ),
                    ...
                    [
                      "时间:",
                      Commonfun.getLocalTime(item.latest_location.loc_time),
                    ],
                  ].concat(temp),
                };
                that.data.selectCarStartDatetime = Commonfun.getStartTime(
                  new Date(item.latest_location.loc_time * 1000)
                );
                // 从localstorage中获取应该显示到列表中的字段，entitydesc或者entityname
                let entityOption =
                  localStorage["entityOption" + that.data.serviceId];
                if (entityOption === "byname" || entityOption === undefined) {
                  that.data.selectCompleteEntities[index]["entity_print"] =
                    item.entity_name;
                  if (!!item.entity_desc) {
                    that.data.selectCompleteEntities[index]["infor"].unshift([
                      "描述",
                      item.entity_desc,
                    ]);
                  }
                ...
                that.data.selectCompleteEntities[index]["entity_name"] =
                  item.entity_name;
                that.data.selectCompleteEntities[
                  index
                ].entity_status = entity_status;
  ...
  setAllEntities: function (data) {
    ...
    data.entities.map(function (item, index) {
      ...
      if (entityOption === "byname" || entityOption === undefined) {
        ...
        that.data.allEntities[index][5] = item.entity_desc
          ? item.entity_desc
          : "无";
      }
    });
  ...
  setOfflineEntities: function (data) {
    ...
    data.entities.map(function (item, index) {
      ...
      if (entityOption === "byname" || entityOption === undefined) {
        ...
        that.data.offlineEntities[index][5] = item.entity_desc
          ? item.entity_desc
          : "无";
      }
    });
  ...
  setOnlineEntities: function (data) {
    ...
    data.entities.map(function (item, index) {
      ...
      if (entityOption === "byname" || entityOption === undefined) {
        ...
        that.data.onlineEntities[index][5] = item.entity_desc
          ? item.entity_desc
          : "无";
      }
    });
        ...
        if (data.status === 0) {
          ...
          data.entities.map(function (item) {
            var paramsd = {
              ...
              process_option:
                "need_denoise=" +
                that.data.trackProcess.need_denoise +
                "," +
                "need_vacuate=" +
                that.data.trackProcess.need_vacuate +
                "," +
                "need_mapmatch=" +
                that.data.trackProcess.need_mapmatch +
                "," +
                "transport_mode=" +
                that.data.transport_mode[
                  that.data.trackProcess.transport_mode - 1
                ],
            };
            Urls.jsonp(Urls.getDistance, paramsd, function (datad) {
              if (datad.status === 0) {
                ...
                if (
                  ++that.data.tracklistloadedConunt === that.data.trackListSize
                ) {
                ...
                var tempTimeArr = [];
                var partTime = Math.floor(
                  (that.data.end_time - that.data.start_time) / 6
                );
                for (var i = 0; i < 6; i++) {
                  tempTimeArr[i] = {
                ...
                tempTimeArr.map(function (item_time) {
                  ...
                  Urls.jsonp(Urls.getDistance, param_time, function (
                    data_time
                  ) {
                    if (data_time.status === 0) {
                      ...
                      distance_time[key].total =
                        distance_time[key].total + data_time.distance;
                      if (distance_time[key].distance.length === 6) {
                        var trackDistance = (
                          distance_time[key].total / 1000
                        ).toFixed(1);
                        that.setTracklistDistance(
                          trackDistance,
                          item.entity_name
                        );
                        that.trigger("tracklist", that.getTracklist());
                        if (
                          ++that.data.tracklistloadedConunt ===
                          that.data.trackListSize
                        ) {
                          that.trigger("tracklistloaded");
                          that.data.tracklistloadedConunt = 0;
                          if (that.data.triggerselecttrack) {
  ...
  setTracklist: function (data) {
    ...
    data.entities.map(function (item, index) {
      if (item.trackDistance != undefined) {
        // alert(1);
        that.data.trackList.push([
          item.entity_name,
          item.trackDistance,
          item.trackDistance > 0 ? 0 : 1,
        ]);
      } else {
        ...
        if (entityOption === "byname" || entityOption === undefined) {
          ...
          that.data.trackList[index]["print"] = item.entity_desc
            ? item.entity_desc
            : "无";
        }
  ...
  onSelecttrack: function (data) {
    ...
    var params = {
      ...
      process_option:
        ...
        "," +
        "transport_mode=" +
        that.data.transport_mode[that.data.trackProcess.transport_mode - 1],
    };
    var count = 1;
        ...
        Urls.jsonp(Urls.getTrack, paramsearch, function (data) {
          ...
          if (data.status === 0) {
            ...
            if (that.data.trackRouteDataCount === 12) {
              ...
              for (var i = 0; i < 12; i++) {
                that.data.trackRoutePointData = that.data.trackRoutePointData.concat(
                  that.data.trackRouteSortData[i].data.points
                );
              }
    ...
    var reTrackRoute = function (paramsr) {
      ...
      var search = function (paramsearch, counta) {
        Urls.jsonp(Urls.getstaypoint, paramsearch, function (data) {
          if (data.status === 0) {
            that.data.trackStayRouteSortData.push({
              index: counta,
              data: data,
            });
            if (++that.data.trackStayRouteDataCount === 6) {
              ...
              for (var i = 0; i < 6; i++) {
                if (
                  that.data.trackStayRouteSortData[i].data.stay_points !==
                  undefined
                ) {
                  that.data.trackStayRoutePointData = that.data.trackStayRoutePointData.concat(
                    that.data.trackStayRouteSortData[i].data.stay_points
                  );
                }
              }
    ...
    if (data.status === 0) {
      if (data.result.formatted_address !== "") {
        ...
        address =
          ...
          ", " +
          data.result.addressComponent.country;
      }
    } else {
      address = "地址未解析成功";
    }
